...
Training XRD Denoising Model — versi KAGGLE tanpa argparse
Struktur dataset:
  /kaggle/input/xrd-denoising-dataset/
    └── processed/
        ├── train/clean/
        ├── train/noisy/
        ├── val/clean/
...
try:
    ...
    print("❌ ERROR: Gagal mengimpor modul lokal!")
    print(e)
...
# ============================================================
# CONFIGURASI (SESUAIKAN DI SINI SAJA)
# ============================================================
class Config:
    # Dataset
    ...
    # Training
    EPOCHS = 5          # Ubah ke 100 untuk training penuh
    BATCH_SIZE = 2      # Kurangi jika OOM
    LR = 1e-3
    WEIGHT_DECAY = 1e-5
    MODEL_TYPE = "unet"
    BASE_CHANNELS = 32
    ...
    NUM_WORKERS = 2
    PIN_MEMORY = False
    TEST_MODE = True    # Otomatis aktifkan test mode
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch_desc=""):
    model.train()
    total_loss = 0.0
    pbar = tqdm(loader, desc=f"{epoch_desc} Train", leave=False, dynamic_ncols=True)
...
@torch.no_grad()
def validate(model, loader, criterion, device, epoch_desc=""):
    model.eval()
    total_loss = 0.0
    pbar = tqdm(loader, desc=f"{epoch_desc} Val", leave=False, dynamic_ncols=True)
    for noisy, clean in pbar:
        noisy = noisy.to(device, non_blocking=True)
        clean = clean.to(device, non_blocking=True)
        output = model(noisy)
...
def main():
    ...
    epoch_pbar = tqdm(range(cfg.EPOCHS), desc="Overall Progress", position=0, dynamic_ncols=True)
    for epoch in epoch_pbar:
        epoch_desc = f"Epoch {epoch+1}/{cfg.EPOCHS}"

        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, device, epoch_desc)
        val_loss = validate(model, val_loader, criterion, device, epoch_desc)
